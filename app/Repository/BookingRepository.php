...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($cuser->is('customer')) {
            // for customers, retrieve jobs with specific statuses and relationships
            $jobs = $cuser->jobs()
            ...
            // for translators, get their jobs with status 'new'
        ...
        if ($jobs) {
            ...
            // check a specific condition for each normal job
     ...
     public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser->is('customer')) {
            // for customers, retrieve completed jobs with specific relationships
            $jobs = $cuser->jobs()
    ...
    public function store($user, $data)
    {
        ...
        $requiredFields = ['from_language_id', 'immediate', 'due_date', 'due_time'];
        ...
        // simplify gender and certified options
        $data['gender'] = in_array('male', $data['job_for']) ? 'male' : (in_array('female', $data['job_for']) ? 'female' : null);
        $data['certified'] = in_array('certified', $data['job_for']) ? 'yes' : (in_array('certified_in_law', $data['job_for']) ? 'law' : (in_array('certified_in_helth', $data['job_for']) ? 'health' : 'normal'));
    ...
    public function storeJobEmail($data)
    {
        ...
        // check if additional address information is provided, otherwise use user's default data
        ...
        // determine the recipient's email and name
    ...
    public function jobToData($job)
    {
        ...
        // map gender and certified values to readable strings
        $genderMap = [
            'male' => 'Man',
            'female' => 'Kvinna',
        ];
     ...
     */
    public function jobEnd($jobId, $userId)
    {
        $completedDate = now();
        ...
        $sessionTimeText = $sessionTimeExplode[0] . ' tim ' . $sessionTimeExplode[1] . ' min';
        ...
        $this->sendSessionEndedEmail($email, $name, $subject, 'emails.session-ended', $data);
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        // determine job_type based on translator_type
        $job_type = ($translator_type == 'professional') ? 'paid' :
                    (($translator_type == 'rwstranslator') ? 'rws' : 'unpaid');
        ...
        $job_ids = collect($job_ids)->filter(function ($job) use ($user_id) {
            $jobuser_id = $job->user_id;
            $checktown = Job::checkTowns($jobuser_id, $user_id);
    ...
    {
        $translatorArray = $this->getSuitableTranslators($job, $data, $exclude_user_id, false);
        $delayedTranslatorArray = $this->getSuitableTranslators($job, $data, $exclude_user_id, true);

        $msgContents = $this->getMessageContent($data);
        $msgText = ["en" => $msgContents];

        $this->sendPushNotification($translatorArray, $job->id, $data, $msgText, false);
        $this->sendPushNotification($delayedTranslatorArray, $job->id, $data, $msgText, true);
    }

    private function getSuitableTranslators($job, $data, $excludeUserId, $delayPush)
    {
        $suitableTranslators = [];
                ...
                if ($job->id == $oneJob->id) {
                    ...
                    if ($jobForTranslator == 'SpecificJob') {
                        ...
                        if ($jobChecker != 'userCanNotAcceptJob') {
                            $targetArray = $delayPush ? &$suitableTranslators['delayed'] : &$suitableTranslators['immediate'];
                            $targetArray[] = $oneUser;
                        }
                    }
                }
    ...
    private function getMessageContent($data)
    {
        $language = TeHelper::fetchLanguageFromJobId($data['from_language_id']);
        $duration = $data['duration'];
        $due = $data['due'];
        $msgContents = ($data['immediate'] == 'no')
            ? "Ny bokning för $language tolk $duration min $due"
            : "Ny akutbokning för $language tolk $duration min";

        return $msgContents;
    }

    private function sendPushNotification($users, $jobId, $data, $msgText, $delay)
    {
        ...
        foreach ($users as $user) {
            $this->sendPushNotificationToSpecificUser($user, $jobId, $data, $msgText, $delay);
        }
    }
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        foreach ($translators as $translator) {
            $this->sendSMSToTranslator($translator, $message);
        }
    ...
    private function prepareSMSMessage($job)
    {
        ...
        $jobId = $job->id;
        $city = $this->getJobCity($job);

        $messageTemplate = trans('sms.phone_job', ['date' => $date, 'time' => $time, 'duration' => $duration, 'jobId' => $jobId]);

        if ($job->customer_physical_type == 'yes' && $job->customer_phone_type == 'no') {
            // It's a physical job
            $messageTemplate = trans('sms.physical_job', ['date' => $date, 'time' => $time, 'town' => $city, 'duration' => $duration, 'jobId' => $jobId]);
        }
    ...
    private function getJobCity($job)
    {
        return $job->city ?: UserMeta::where('user_id', $job->user_id)->value('city');
    }
    ...
    {
        $logger = $this->setupLogger($job_id, $users, $data, $msg_text, $is_need_delay);
        ...
        $data = $this->prepareNotificationData($job_id, $data, $msg_text);

        if ($is_need_delay) {
            $next_business_time = DateTimeHelper::getNextBusinessTimeString();
            $data['send_after'] = $next_business_time;
        }

        $fields = $this->prepareNotificationFields($onesignalAppID, $user_tags, $data);
        
        $this->sendNotificationRequest($fields, $onesignalRestAuthKey, $logger);
    }

    private function setupLogger($job_id, $users, $data, $msg_text, $is_need_delay)
    {
        $logger = new Logger('push_logger');
        $logContext = [$users, $data, $msg_text, $is_need_delay];
        $logger->pushHandler(new StreamHandler(storage_path('logs/push/laravel-' . date('Y-m-d') . '.log'), Logger::DEBUG));
        $logger->pushHandler(new FirePHPHandler());
        $logger->addInfo('Push send for job ' . $job_id, $logContext);
        return $logger;
    }

    private function prepareNotificationData($job_id, $data, $msg_text)
    {
        $notificationData = $data;
        $notificationData['job_id'] = $job_id;
        $notificationData['contents'] = $msg_text;
        $notificationData['title'] = ['en' => 'DigitalTolk'];
        
        if ($data['notification_type'] == 'suitable_job') {
            $notificationData['android_sound'] = 'normal_booking';
            $notificationData['ios_sound'] = 'normal_booking.mp3';
            
            if ($data['immediate'] == 'yes') {
                $notificationData['android_sound'] = 'emergency_booking';
                $notificationData['ios_sound'] = 'emergency_booking.mp3';
            }
        }
    ...
    private function prepareNotificationFields($onesignalAppID, $user_tags, $data)
    {
        return json_encode([
    ...
    private function sendNotificationRequest($fields, $onesignalRestAuthKey, $logger)
    {
        $ch = curl_init();
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $this->handleJobUpdates($job, $current_translator, $changeTranslator, $langChanged, $changeDue['dateChanged']);
    }
    ...
    {
        $logMessage = 'USER #' . $cuser->id . '(' . $cuser->name . ')' . ' has been updated booking <a class="openjob" href="/admin/jobs/' . $id . '">#' . $id . '</a> with data: ';
        $this->logger->addInfo($logMessage, $log_data);
    }

    private function handleJobUpdates($job, $current_translator, $changeTranslator, $langChanged, $dateChanged)
    {
        if ($dateChanged) {
...
//        }
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            $hoursDifference = $job->withdraw_at->diffInHours($job->due);
            ...
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    $this->notifyCustomerOnCancellation($customer, $job);
                }
    ...
    private function notifyCustomerOnCancellation($customer, $job)
    {
        $data = [];
        $data['notification_type'] = 'job_cancelled';
        $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
    ...
    private function getUserLanguages($userId)
    {
        ...
        return collect($languages)->pluck('lang_id')->all();
    }
    ...
    public function endJob($post_data)
    {
        ...
        $this->sendSessionEndedEmails($job, $sessionTime, 'faktura');
        ...
        if ($translator) {
            $this->sendSessionEndedEmails($translator, $sessionTime, 'lön');
            $translator->completed_at = $completedDate;
            $translator->completed_by = $post_data['user_id'];
            $translator->save();
        }

        Event::fire(new SessionEnded($job, ($post_data['user_id'] == $job->user_id) ? $translator->user_id : $job->user_id));
    ...
    private function sendSessionEndedEmails($recipient, $sessionTime, $forText)
    {
        $user = $recipient->user;
        $email = !empty($recipient->user_email) ? $recipient->user_email : $user->email;
        $name = $user->name;
        $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $recipient->job_id;
